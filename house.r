...
# df$Floor.Current <- strsplit(df$Floor, split=" ")[[]][1]

# df$Floor.Total <- strsplit(df$Floor, split=" ")[[]][4]
...
rent_hist <- ggplot(df, aes(x=Rent, colour=City)) + geom_histogram() + ggtitle("City Rent Distribution") + labs(y="Frequency", x="Rent City") + theme_light()
rent_den <- ggplot(df, aes(x=Rent, colour=City)) + geom_density() + ggtitle("Tenant Prefered Rent Distribution") + labs(y="Density", x="Rent City") + theme_light()
...
grid.arrange(rent_hist, rent_den, kolkota_rent_hist, kolkota_den, mumbai_rent_hist, mumbai_den, delhi_rent_hist, delhi_den, chennai_rent_hist, chennai_den, hyberabad_rent_hist, hyberabad_den, nrow=6, ncol=2)
# GDP and GDP per pacipta of each city

# Analysis 2-9: Plot the boxplot of city
ggplot(df, aes(x=City, y=Rent)) + geom_boxplot() + ggtitle("Boxplot of City and Rent Price") + labs(y="Rent Price", x="City") + theme_light()

# Analaysis 2-10 Plot features againts Kolkata and Mumbai
df_kolkota_mumbai <- df[df$City %in% c("Kolkata", "Mumbai"), ]

bhk_km_df <- aggregate(Rent ~ Bathroom + City, data = df_kolkota_mumbai, FUN = length)

bhk_km_df

cor(df_kolkota_mumbai[, numerical_features])

size_scatter <- ggplot(df_kolkota_mumbai, aes(color=City, shape=City, y=Rent, x=Size)) + geom_point(size=3)
bhk_box <- ggplot(df_kolkota_mumbai, aes(fill=City, y=Rent, x=BHK)) + geom_boxplot()
bathroom_box <- ggplot(df_kolkota_mumbai, aes(fill=City, y=Rent, x=Bathroom)) + geom_boxplot()
furnishing_box <- ggplot(df_kolkota_mumbai, aes(fill=City, y=Rent, x=Furnishing.Status)) + geom_boxplot()
tenants_box <- ggplot(df_kolkota_mumbai, aes(fill=City, y=Rent, x=Tenant.Preferred)) + geom_boxplot()
area_box <- ggplot(df_kolkota_mumbai, aes(fill=City, y=Rent, x=Area.Type)) + geom_boxplot()

grid.arrange(size_scatter, bhk_box, bathroom_box, furnishing_box, tenants_box, area_box, nrow=3, ncol=2)
# Internal (feature from the dataset) and external (features outside of the dataset) factors
...
rent_hist <- ggplot(df, aes(x=Rent, colour=Tenant.Preferred)) + geom_histogram() + ggtitle("Tenant Prefered Rent Distribution") + labs(y="Frequency", x="Rent Tenant Prefered") + theme_light()
...
rent_den <- ggplot(df, aes(x=Rent, colour=Tenant.Preferred)) + geom_density() + ggtitle("Tenant Prefered Rent Distribution") + labs(y="Density", x="Rent Tenant Prefered") + theme_light()
...
grid.arrange(rent_hist, rent_den, bachelor_rent_hist, bachelor_rent_den, family_rent_hist, family_rent_den, bachelor_family_rent_hist, bachelor_family_rent_den, nrow=4, ncol=2)

# Analysis 3-3: Plot a boxplot for each tenant prefered
...
rent_hist <- ggplot(df, aes(x=Rent, colour=Furnishing.Status)) + geom_histogram() + ggtitle("Furnishing Status Rent Distribution") + labs(y="Frequency", x="Furnishing Status Prefered") + theme_light()
...
rent_den <- ggplot(df, aes(x=Rent, colour=Furnishing.Status)) + geom_density() + ggtitle("Furnishing Status Rent Distribution") + labs(y="Density", x="Furnishing Status Prefered") + theme_light()
...
grid.arrange(rent_hist, rent_den, unfurnished_rent_hist, unfurnished_rent_den, semi_unfurnished_rent_hist, semi_furnished_rent_den, furnished_rent_hist, furnished_rent_den, nrow=4, ncol=2)

# Analysis 4-3: Plot a boxplot for each furnishing type
